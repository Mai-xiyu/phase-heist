...
> 文档版本 0.6 — 本次更新（沉浸感）：交互点道具化（电话/扩音器/钱袋/录音机/拒马/炸药包等实体道具替代地面大圆盘）、人质直接按 E 释放、警用无线电语音频道（警察互通带电音、劫匪听不到）、银行建筑立面与大厅精修（三角楣/柱头/窗框/吊梁/红毯/品牌墙/挂钟）、警灯红蓝交替闪烁、喇叭/电话情境音效。
>
> 0.5：可驾驶载具、画面美化（SSAO/Glow/雾/ACES）、HUD 重排、程序化车模重做。
>
...
### 交互系统（v0.4 近距触发 / v0.6 道具化）

- **近距触发**：走进交互范围按 E 即可，无需准星对准。
- **实体道具载体**（v0.6）：每个交互点是真实道具——柜台电话、警用扩音器架、金库钱袋堆、卷盘录音机、人质椅、换装衣架、警用拒马、贴门炸药包、处决台、井盖撬棍（`ObjectiveProps`）。地面只留细环 + 悬浮旋转菱形标记。
- **对 NPC 交互**（v0.6）：释放人质 = 走到人质身边直接按 E（`BankNpc` 实现 `IInteractable`），服务端按"离操作者最近"标记对应人质。
- **HUD 提示**：屏幕中下方实时显示「[E] 当前可执行动作」。
- **可用性同源**：`BankActionRules` 同时驱动提示与裁决——不可用交互点（错误阵营/阶段）变暗且按下无效。

### 队伍语音与无线电（v0.6）

- **警察 → 警察**：全图无线电频道（2D，无距离衰减），经 `Radio` 总线加 **高通 420Hz + 低通 3.1kHz + 过载失真 + 压缩** 的对讲机电音，劫匪完全听不到此通道。
- **警察 → 劫匪 / 劫匪 → 任何人**：仍是 3D 近距离语音——警察当面喊话劫匪能听见，隔街则听不见。
- 路由在接收端按双方阵营自动选择，发送端协议不变。
...
- [x] 交互点道具化（10 种程序化道具）+ 细环/菱形标记替代大圆盘
- [x] 人质 NPC 直接交互释放（按最近人质精确标记）
- [x] 警用无线电：队内全图语音 + 电音 DSP 总线，劫匪不可闻
- [x] 银行立面（三角楣/柱头柱础/窗框竖梃/基座带）与大厅（吊梁/红毯/品牌墙/挂钟）精修
- [x] 警灯红蓝交替闪烁（`FlashingLight`）、喇叭啸叫/电话拨号情境音效
